...
import { Category, Solution, PaginationInfo } from '@/types';
...
export default function SolutionsPage() {
    ...
    const fetchInitialData = async () => {
        setLoading(true);
        try {
            const [catsRes, solutionsRes] = await Promise.all([
                apiFetch('/categories'),
                apiFetch('/solutions')
            ]);

            if (catsRes.success) setCategories(catsRes.data);
            if (solutionsRes.success) setSolutions(solutionsRes.data);
        } catch (err) {
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const data = await apiFetch(`/categories/${selectedCategoryId}/solutions`, {
                method: 'POST',
                body: JSON.stringify({})
            });
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const data = await apiFetch(`/solutions/${editingSolution._id}`, {
                method: 'PUT',
                body: JSON.stringify({
            ...
            alert(err.message || 'Failed to update solution');
        }
    };
    ...
    const handleToggleStatus = async (solution: Solution) => {
        try {
            const data = await apiFetch(`/solutions/${solution._id}`, {
                method: 'PUT',
                body: JSON.stringify({ isActive: !solution.isActive })
            });
    ...
    const openEditModal = (solution: Solution) => {
        ...
        const parentId = typeof solution.category === 'string' ? solution.category : solution.category._id;
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-slate-800">Solutions Details</h1>
                <button
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse text-sm min-w-[600px]">
                        ...
                        <tbody>
                            {loading ? (
                                <tr>
                                    <td colSpan={4} className="py-8 text-center text-slate-500">Loading solutions...</td>
                                </tr>
                            ) : solutions.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="py-8 text-center text-slate-500">No solutions found.</td>
                                </tr>
                            ) : (
                                solutions.map((solution, index) => {
                                    const categoryName = typeof solution.category === 'object' ? solution.category.name : 'Unknown Category';

                                    return (
                                        <tr key={solution._id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${!solution.isActive ? 'opacity-60 bg-slate-50/50' : ''}`}>
                                            ...
                                            <td className="py-3 px-6">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-1.5 text-white rounded transition-colors ${solution.isActive ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'}`}
                                                        title={solution.isActive ? "Deactivate" : "Activate"}
                                                    >
                                                        {solution.isActive ? <Check size={14} /> : <X size={14} />}
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
            ...
            {editingSolution && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white w-full max-w-lg rounded shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="bg-teal-500 text-white px-6 py-4 flex justify-between items-center">
                            <h2 className="text-xl font-medium">Edit Solution</h2>
                            <button
