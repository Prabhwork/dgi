...
export const requestForToken = async () => {
  ...
  try {
    ...
    if (currentToken) {
        console.log("FCM Token secured (Food Dash):", currentToken);
        
        // Auto-register with backend if possible
        const apiBase = process.env.NEXT_PUBLIC_API_URL || "http://localhost:3004/api";
        ...
        if (token) {
          try {
            ...
            console.log("[FCM] Registered token with Food backend");
          } catch (regErr) {
            console.warn("[FCM] Food Backend registration failed", regErr);
          }
        }
        return currentToken;
    }
