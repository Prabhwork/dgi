...
import { STATS, INITIAL_ORDERS } from '@/lib/mockData';
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-400 mt-1">Real-time business performance for Suman Lata Bhadola Marg</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] font-semibold text-slate-400">{order.time}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-medium text-slate-500 truncate max-w-[180px] block">{order.items}</span>
                      </td>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6">
            {[
              { name: 'Pizza', count: '124 Orders', percentage: 75, color: '#dc2626' },
              { name: 'Burger', count: '89 Orders', percentage: 55, color: '#f59e0b' },
              { name: 'Drinks', count: '56 Orders', percentage: 40, color: '#10b981' },
            ].map((cat) => (
