...
export const requestForToken = async () => {
  ...
  try {
    ...
    if (currentToken) {
        console.log("FCM Token secured (Food Admin):", currentToken);
        
        // Auto-register with backend if possible
        const apiBase = process.env.NEXT_PUBLIC_FOOD_API_URL || "http://localhost:3004/api";
        ...
        if (token) {
          try {
            ...
            console.log("[FCM] Registered token with Food Admin backend");
          } catch (regErr) {
            console.warn("[FCM] Food Admin Backend registration failed", regErr);
          }
        }
        return currentToken;
    }
  } catch (err) {
    console.log("Token extraction failed", err);
  }
};
